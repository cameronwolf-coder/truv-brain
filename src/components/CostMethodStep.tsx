            ...
            <div className="flex flex-col gap-3">
                {costMethods.map((method, index) => (
                    <motion.button
                        ...
                        className={`relative p-5 rounded-xl border text-left transition-all overflow-hidden ${selectedMethod === method.id
                                ? 'border-truv-blue bg-white'
                                : 'border-border hover:border-gray-300'
                            }`}
                    >
                        {/* Left accent bar */}
                        <div className={`absolute left-0 top-0 bottom-0 w-1 transition-colors ${selectedMethod === method.id ? 'bg-truv-blue' : 'bg-transparent'
                            }`} />

                        <div className="flex items-center gap-4">
                            {/* Radio button */}
                            <div className={`w-[22px] h-[22px] rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${selectedMethod === method.id
