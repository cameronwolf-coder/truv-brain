...
import { cached, SEVEN_DAYS, STALE_TTL, FRESH_TTL } from './_lib/cache';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    // Cache per workflow+page. The TTL callback checks the most recent
    // recipient timestamp to decide fresh (15 min) vs stale (30 days).
    const result = await cached(
      `ep:detail:${workflow}:${offset}:${limit}`,
      (val) => {
        if (!val || val.total === 0) return FRESH_TTL;
        const newest = val.recipients[0]?.summary?.last_activity || 0;
        return (now - newest) > SEVEN_DAYS ? STALE_TTL : FRESH_TTL;
      },
      async () => {
        ...
        // Step 2: Build engagement lookup from SendGrid
        ...
        return { workflow_key: workflow, recipients, total: totalCount };
      }
    );
