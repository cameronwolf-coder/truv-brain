export const getToolDefinitions = () => [
    ...
    },
    {
        ...
        description: "List all workflows in the workspace. Returns workflow IDs, triggers, and step counts. Note: fetches details for each workflow so use a small limit for quick results.",
        inputSchema: {
            type: "object",
            properties: {
                limit: {
    ...
    },
    {
        ...
        description: "Get details of a specific workflow by ID (starts with p_). Returns triggers, steps, and configuration.",
        inputSchema: {
            type: "object",
            properties: {
                workflow_id: {
    ...
    },
    {
        ...
        description: "Create a new workflow from a template. Requires a project ID and template ID from Pipedream.",
        inputSchema: {
            type: "object",
            properties: {
                project_id: {
    ...
    },
    {
        ...
        description: "Trigger a workflow via its HTTP endpoint URL. Only works for workflows with HTTP triggers.",
        inputSchema: {
            type: "object",
            properties: {
                endpoint_url: {
    ...
    },
    {
        ...
        description: "List all subscriptions (connections between emitters and listeners) in the workspace",
        inputSchema: {
            type: "object",
            properties: {
                limit: {
    ...
    },
    {
        ...
        description: "Subscribe a listener (workflow/source) to events from an emitter (source/trigger)",
        inputSchema: {
            type: "object",
            properties: {
                emitter_id: {
    ...
    },
    {
        ...
        description: "Search the Pipedream component registry for available actions and triggers",
        inputSchema: {
            type: "object",
            properties: {
                query: {
...
export async function handleToolCall(service, name, args = {}) {
    let result;
    switch (name) {
        ...
        case "pipedream_list_workflows":
            result = await service.listWorkflows(args.limit || 10);
            break;
        case "pipedream_get_workflow":
            result = await service.getWorkflow(args.workflow_id);
            break;
        ...
        case "pipedream_update_workflow":
            result = await service.updateWorkflow(args.workflow_id, args.active);
            break;
        case "pipedream_trigger_workflow":
            result = await service.triggerWorkflow(args.endpoint_url, args.payload);
            break;
        case "pipedream_list_sources":
            result = await service.listSources(args.limit || 10);
            break;
        case "pipedream_get_source_events":
            result = await service.getSourceEvents(args.source_id, args.limit || 10);
            break;
        case "pipedream_update_source":
            result = await service.updateSource(args.source_id, args.active);
            break;
        case "pipedream_delete_source_events":
            result = await service.deleteSourceEvents(args.source_id);
            break;
        case "pipedream_list_subscriptions":
            result = await service.listSubscriptions(args.limit || 100);
            break;
        case "pipedream_create_subscription":
            result = await service.createSubscription(args.emitter_id, args.listener_id, args.event_name);
            break;
        case "pipedream_delete_subscription":
            result = await service.deleteSubscription(args.subscription_id);
            break;
        case "pipedream_search_components":
            result = await service.searchComponents(args.query, args.limit || 10);
            break;
        case "pipedream_list_apps":
            result = await service.listApps(args.query, args.limit || 10);
            break;
        default:
